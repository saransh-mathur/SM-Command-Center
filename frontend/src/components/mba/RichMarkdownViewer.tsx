...
export const RichMarkdownViewer: React.FC<RichMarkdownViewerProps> = ({ content }) => {
 ...
 if (part.startsWith('`') && part.endsWith('`')) {
 return <code key={i} className="px-1.5 py-0.5 rounded bg-slate-900 text-cyan-300 font-mono text-[11px] ">{part.slice(1, -1)}</code>;
 }
 if (part.startsWith('$') && part.endsWith('$')) {
 return <span key={i} className="px-1.5 py-0.5 rounded bg-amber-950/40 text-amber-300 font-mono text-[11px] border-amber-500/20">{part.slice(1, -1)}</span>;
 }
 return part;
 });
 };
 ...
 return (
 <div key={index} className="my-4 rounded-xl bg-slate-950 border-emerald-500/30 overflow-hidden shadow-glass">
 ...
 return (
 <div key={index} className={`my-3 rounded-xl overflow-hidden ${isLatex ? 'bg-amber-950/20 border-amber-500/30' : 'bg-slate-950 '}`}>
 <div className={`flex items-center justify-between px-3.5 py-1.5 border-b font-mono text-[10px] ${isLatex ? 'bg-amber-950/40 border-amber-500/20 text-amber-300' : 'bg-slate-900 text-slate-400'}`}>
 <span className="flex items-center gap-1.5">
 ...
 return (
 <div key={index} className={`pt-2 pb-1 border-b ${isMainUnit ? 'mb-3' : 'mb-2'}`}>
 ...
 <h3 className="text-sm font-bold text-cyan-300 font-mono flex items-center gap-2 border-b pb-1.5">
 ...
 return (
 <div key={index} className="p-3.5 my-2.5 rounded-xl bg-slate-900/80 border-l-4 border-l-emerald-500 text-xs text-slate-300 leading-relaxed shadow-sm">
 ...
 return (
 <div key={index} className="my-4 overflow-x-auto rounded-xl shadow-glass">
 <table className="w-full text-left border-collapse font-sans text-xs">
 <thead>
 <tr className="bg-slate-900/90 border-b ">
 {headers.map((h, hIdx) => (
 <th key={hIdx} className="p-3 text-cyan-300 font-mono font-bold text-[11px] tracking-wide border-r last:border-r-0">
 ...
 {row.map((cell, cIdx) => (
 <td key={cIdx} className="p-3 text-slate-300 leading-relaxed border-r last:border-r-0">
