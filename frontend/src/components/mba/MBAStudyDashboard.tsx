...
export const MBAStudyDashboard: React.FC = () => {
 ...
 {/* Top Banner */}
 <div className="bg-slate-900/80 backdrop-blur-md rounded-2xl p-5 shadow-glass flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
 ...
 <div className="px-3 py-1.5 rounded-xl bg-slate-950/60 flex items-center gap-1.5 text-amber-400">
 <Flame className="w-4 h-4" />
 <span className="font-bold">5-Day Study Streak</span>
 </div>
 <div className="px-3 py-1.5 rounded-xl bg-slate-950/60 flex items-center gap-1.5 text-cyan-400">
 ...
 : 'bg-slate-950/40 hover:bg-slate-900/60 hover:border-slate-700'
 }`}
 >
 <div>
 ...
 {/* Dual Mode Switcher */}
 <div className="flex items-center p-1 rounded-xl bg-slate-950 font-mono text-xs">
 <button
 ...
 className={isFullscreen ? "bg-slate-950 p-4 sm:p-6 w-full h-full overflow-y-auto flex flex-col" : ""}
 >
 {loadingNotes ? (
 ...
 : 'bg-slate-950/40 text-slate-400 hover:text-slate-200 hover:border-slate-700'
 }`}
 >
 ...
 {/* Unit Content Panel */}
 <div className={`flex-1 p-5 rounded-xl bg-slate-950/70 space-y-5 relative ${
 ...
 className="absolute top-4 right-4 p-2 rounded-xl bg-slate-800/80 hover:bg-slate-700 text-slate-300 transition-all shadow-sm z-10"
 ...
 <p className="text-xs text-slate-300 mt-2 leading-relaxed bg-slate-900/60 p-3 rounded-xl ">
 {deep?.overview}
 </p>
 </div>
 ...
 {raw ? (
 <div className="p-5 rounded-2xl bg-slate-900/90 shadow-glass">
 ...
 <div key={idx} className="p-3.5 rounded-xl bg-slate-900/70 ">
 ...
 <div className="p-4 rounded-xl bg-slate-900/80 space-y-2">
 ...
 <div key={idx} className="p-2.5 rounded-lg bg-slate-950 text-amber-200 font-mono text-xs border-amber-500/20">
 {f}
 </div>
 ))}
 </div>
 ...
 <div className="p-4 rounded-xl bg-slate-900/80 space-y-2.5">
 ...
 <div className="p-3.5 rounded-xl bg-slate-900/80 space-y-2">
 ...
 <div key={idx} className="p-2 rounded bg-slate-950 text-emerald-200 font-mono text-xs border-emerald-500/20">
 {f}
 </div>
 ))}
 </div>
 ...
 className="px-2.5 py-1 rounded-lg bg-slate-950/60 hover:bg-slate-900 text-[11px] text-slate-300 hover:text-cyan-300 flex-shrink-0 transition-all text-left"
 >
 • {p}
 </button>
 ))}
 ...
 {/* Chat History Box */}
 <div className="min-h-[380px] max-h-[480px] overflow-y-auto rounded-xl p-4 bg-slate-950/80 flex flex-col gap-3 font-sans">
 {messages.map((msg) => (
 <div
 key={msg.id}
 ...
 : 'bg-slate-900/90 text-slate-200 rounded-tl-none whitespace-pre-line'
 }`}
 >
 {msg.text}
 </div>
 ...
 {isTyping && (
 <div className="self-start p-3 rounded-2xl bg-slate-900 text-xs font-mono text-cyan-400 flex items-center gap-2">
 ...
 className="flex-1 px-4 py-2.5 rounded-xl bg-slate-900 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500 font-sans"
 />
 <button
 ...
 {activeTab === 'timer' && (
 <div className="max-w-md mx-auto p-6 rounded-2xl bg-slate-950/80 text-center font-mono">
 ...
 className="p-2.5 rounded-xl bg-slate-900 text-slate-400 hover:text-slate-200"
 title="Reset"
 >
      ...
      {sessions.length === 0 ? (
        ...
        <div className="space-y-4">
          {sessions.map((session) => (
            <div key={session.id} className="p-5 rounded-2xl bg-slate-900/50 border border-slate-800 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-slate-800 text-slate-400">
                    {new Date(session.created_at).toLocaleDateString()}
                  </span>
                  <h4 className="text-sm font-bold text-slate-200 mt-1">{session.subject}</h4>
                </div>
              ...
              <p className="text-xs text-slate-400 font-mono">{session.topic}</p>
              
              {session.ai_notes_snapshot && session.ai_notes_snapshot !== "No notes captured." && (
                <div className="mt-3 p-4 rounded-xl bg-slate-950 border border-slate-800/80 max-h-64 overflow-y-auto custom-scrollbar">
