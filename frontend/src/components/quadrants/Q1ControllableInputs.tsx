...
export const Q1ControllableInputs: React.FC = () => {
  ...
  return (
    <div className="glass-panel rounded-2xl p-5 border border-slate-800/80 shadow-glass flex flex-col justify-between h-full relative overflow-hidden group hover:border-slate-700 transition-all bg-slate-900/60">
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-slate-900/60 border border-slate-800 hover:border-slate-700 text-slate-400 hover:text-slate-200 transition-all cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-4">
          <motion.button
            ...
            className="p-3 rounded-xl bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 text-slate-950 font-black text-xs flex items-center justify-between shadow-glow-emerald border border-emerald-400/40 cursor-pointer"
          >
            <div className="flex items-center gap-2">
          ...
          <motion.button
            ...
            className="p-3 rounded-xl bg-slate-950 border border-slate-700 hover:border-slate-600 text-slate-200 font-bold text-xs flex items-center justify-between cursor-pointer"
          >
            <div className="flex items-center gap-2">
        ...
        <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800/80 mb-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-slate-900 border border-emerald-500/40 flex items-center justify-center text-xs font-mono font-bold text-emerald-300">
              {dailyScore}%
            </div>
            <div>
          ...
          <div className="w-36 bg-slate-900 h-2 rounded-full overflow-hidden border border-slate-800 hidden sm:block">
            <div 
        ...
        <div className="space-y-2.5 mb-4">
          {items.map((item) => {
            ...
            return (
              <div
                key={item.key}
                className={`p-3 rounded-xl bg-slate-950/70 border ${
                  isCompleted ? 'border-emerald-500/40 bg-emerald-500/[0.03]' : 'border-slate-800/80'
                } flex flex-col gap-2 transition-all`}
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className={`p-1.5 rounded-lg bg-slate-900 border border-slate-800 ${item.accentColor}`}>
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <button
                      ...
                      className="p-1 rounded bg-slate-900 border border-slate-800 text-emerald-400 hover:text-emerald-300 transition-colors cursor-pointer"
                    >
          ...
          {streakDays.map((s, idx) => (
            <div key={idx} className="flex flex-col items-center gap-0.5">
              <div 
                ...
                className={`w-4 h-4 rounded-sm border ${getHeatmapColor(s.level)} transition-all`}
              />
