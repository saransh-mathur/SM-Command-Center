  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md">
      <motion.div
        ...
        className="w-full max-w-xl glass-panel rounded-2xl border border-emerald-500/50 shadow-2xl p-6 relative overflow-hidden"
      >
        {/* Top Glow Ambient */}
        ...
        {/* Modal Header */}
        <div className="flex items-center justify-between pb-3.5 mb-4 border-b border-slate-800 relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-emerald-300 font-semibold">
                  &lt;3 MINUTE PROTOCOL
                </span>
              </div>
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`p-5 rounded-xl bg-slate-950/70 border ${trackBadge.color.split(' ')[2]} relative overflow-hidden mb-5`}
          >
            <div className="flex items-center justify-between gap-2 mb-3">
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-mono font-bold border ${trackBadge.color}`}>
                <TrackIcon className="w-3.5 h-3.5" />
                {trackBadge.label}
              </span>

              <span className="text-xs font-mono text-slate-400 flex items-center gap-1 bg-slate-900 px-2 py-1 rounded-md border border-slate-800">
                <Timer className="w-3.5 h-3.5 text-cyan-400" />
                <span>Est: {currentRouletteTask.estimatedMinutes} min</span>
              </span>
            </div>

            <h4 className="text-lg font-extrabold text-white mb-1.5 leading-snug">
            ...
            <div className="p-2.5 rounded-lg bg-slate-900/90 border border-slate-800 text-[11px] text-slate-300 font-mono flex items-start gap-2">
        ...
        <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80 mb-5">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              onClick={handleSpin}
              disabled={isSpinning}
              className="px-3.5 py-2 rounded-xl bg-slate-900 border border-slate-700 hover:border-slate-600 text-slate-200 text-xs font-mono font-bold flex items-center gap-2 transition-all cursor-pointer hover:bg-slate-800"
            >
        ...
        <div className="flex items-center justify-between gap-3 pt-3 border-t border-slate-800/80">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={closeModal}
              className="px-3.5 py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs text-slate-400 hover:text-slate-200 font-mono transition-all cursor-pointer"
            >
              Close
            </button>
            <button
