...
export const AdvisorModal: React.FC = () => {
 ...
 className="bg-white rounded-3xl border border-slate-100 shadow-2xl max-w-lg w-full p-6 space-y-5 relative"
 >
 {/* Header */}
 <div className="flex items-center justify-between pb-3 mb-4 border-b ">
 <div className="flex items-center gap-3">
 <div className={`p-2.5 rounded-xl ${advisorConfig.accentBg}`}>
 <Icon className="w-6 h-6" />
 </div>
 <div>
 <div className="flex items-center gap-2">
 <h3 className="text-base font-extrabold text-slate-900">{advisorConfig.name}</h3>
 <span className="text-[10px] font-mono font-semibold px-2 py-0.5 rounded bg-slate-200 text-slate-700">
 ACTIVE
 </span>
 </div>
 <p className="text-xs text-slate-500 font-mono">{advisorConfig.title}</p>
 </div>
 </div>
 ...
 <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100 mb-4 text-xs font-mono">
 <span className="text-[10px] text-slate-500 block font-bold mb-1">
 CORE DIRECTIVE:
 </span>
 <p className="text-slate-800 font-sans italic">"{advisorConfig.quickAdvice}"</p>
 </div>

 {/* Sample Prompt Chips */}
 <div className="mb-4">
 <span className="text-[11px] font-mono text-slate-500 block mb-1.5 font-bold">
 Quick Inquiries:
 </span>
 ...
 className="w-full text-left text-xs p-2 rounded-lg bg-slate-50 border border-slate-100 hover:bg-slate-100 text-slate-700 hover:text-slate-900 font-sans transition-all flex items-center justify-between"
 >
 <span>• {p}</span>
 ...
 className="p-3.5 rounded-xl bg-slate-100 border-cyan-500/30 text-xs font-mono mb-4 text-slate-800 whitespace-pre-line"
 >
 {response}
 </motion.div>
 )}
 ...
 className="flex-1 px-3.5 py-2 rounded-xl bg-slate-100 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-cyan-500 font-mono"
 />
 <button
 onClick={handleAsk}
