  ...
  return (
    <div className="w-full pb-20">
      <div className="flex justify-between items-center mb-4 px-2">
        <h2 className="text-sm font-semibold text-slate-900">WORKSPACE GRID</h2>
        <button 
        ...
        >
          {(layouts?.lg || (Array.isArray(layouts) ? layouts : Object.values(layouts)[0] || [])).map((layoutItem: any) => {
            ...
            const isEditingStyles = isEditMode 
              ? 'border-rose-400 shadow-md cursor-grab active:cursor-grabbing hover:border-rose-500 ring-2 ring-rose-200 ring-offset-2' 
              : 'border-slate-200/80 shadow-sm hover:shadow-md';
            
            return (
              <div 
                key={layoutItem.i} 
                className={`bg-white rounded-2xl border ${isEditingStyles} p-6 transition-all duration-200 overflow-hidden flex flex-col`}
              >
